...
const xhttp = new XMLHttpRequest();
        ...
        urlz='/apiV1/ChapterFromTo?';
        var urlParams = new URLSearchParams(window.location.search);
        if(urlParams.get('From')){
            urlz+='From='+urlParams.get('From')+"&&";
        }
        if(urlParams.get('To')){
            urlz+='To='+urlParams.get('To');
        }
        xhttp.onload = function() 
        {
            ...
            if(xhttp.status==200)
            {
                ...
                var s1Html='<div class="body__hot__hotlist"><a href="">'+'PHÂN LOẠI THEO CHƯƠNG'+'</a></div>';
                var s2 =document.getElementById('listStory');
                var s2Html = '';
                for(var i =0;i<Response.length;i++)
                {
                    s2Html+='<li><a href="intro/'+Response[i].id+'">'+Response[i].StoryName+' ,'+Response[i]['count']+'chương</a></li>';
                }
                s1.innerHTML = s1Html;
                s2.innerHTML = s2Html;
