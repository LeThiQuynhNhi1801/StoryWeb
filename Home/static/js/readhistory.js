...
const xhttp = new XMLHttpRequest();
        ...
        xhttp.onload = function() 
        {
            ...
            if(xhttp.status==200)
            {
                ...
                var s1Html='<div class="body__hot__hotlist"><a href="">LỊCH SỬ ĐỌC</a></div>';
                var s2 =document.getElementById('listStory');
                var s2Html = '';
                for(var i =0;i<Response.length;i++)
                {
                    s2Html+=' <li><a href="/read/'+Response[i]['ids']+'/'+Response[i]['ChapterNumber']+'">'+Response[i]['StoryName']+'</a><p>Bạn đang đọc chương '+Response[i]['ChapterNumber']+' : '+Response[i]['ChapterName']+'</p></li>';
                }
                s1.innerHTML = s1Html;
                s2.innerHTML = s2Html;
    ...
    xhttp.open("GET", "/apiV1/readhistory",false);
    //định dạng gửi đi787
